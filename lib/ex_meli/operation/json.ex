defmodule ExMeli.Operation.JSON do
  @moduledoc """
...
defimpl ExMeli.Operation, for: ExMeli.Operation.JSON do
  @type response_t :: %{} | ExMeli.Request.error_t()

  def perform(operation, config) do
    operation = handle_callbacks(operation, config)
    url = ExMeli.Request.Url.build(operation, config)

    headers = [
      {"x-amz-content-sha256", ""} | operation.headers
    ]

    result = ExMeli.Request.request(operation.http_method, url, operation.data, headers, config)
    parse(result, config)
  end

  def stream!(%ExMeli.Operation.JSON{stream_builder: nil}, _) do
    raise ArgumentError, """
    This operation does not support streaming!
    """
  end

  def stream!(%ExMeli.Operation.JSON{stream_builder: stream_builder}, config_overrides) do
